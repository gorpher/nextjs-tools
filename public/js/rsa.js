function Init() {
    function generateKey(btn) {
        ...
        let encode = document.getElementById('select_encode').value;
        ...
        new Promise(function (resolve) {
            setTimeout(function () {
                if (encode === "base64") {
                    resolve([crypt.getPrivateKeyB64(), crypt.getPublicKeyB64()]);
                    return
                }
    ...
    function encrypt() {
        ...
        if (original_text && public_key) {
            ...
            console.log(crypt);
            cipher_text = crypt.encryptLong(original_text) || ''
        }
    ...
    function decrypt() {
        ...
        if (cipher_text && private_key) {
            let crypt = new JSEncrypt();
            crypt.setPrivateKey(private_key);
            original_text = crypt.decryptLong(cipher_text) || ''
        }
    ...
    document.getElementById('generateBtn').addEventListener('click', (e) => generateKey(e.currentTarget))
